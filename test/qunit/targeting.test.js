...
QUnit.test('getFromConfig', function(assert) {
	...
	assert.deepEqual(
		...
		2,
		'Empty string dfp_targeting returns only default params'
	);
	...
	assert.deepEqual(
		result.targeting,
		'params',
		'Simple params are parsed correctly'
	);
	...
	assert.deepEqual(
		result['s@me'],
		'test',
		'Special characters in targeting key handled'
	);
	assert.deepEqual(
		result.targeting,
		"par@ms$'",
		'Special characters in targeting value handled'
	);
});
...
QUnit.test('OADS_VERSION is not set if the flag is set to false', function(
	assert
) {
	const getVersion = this.stub(this.ads.utils, 'getVersion');
	getVersion.returns('x.y.z');
...
QUnit.test('OADS_VERSION is not set if not provided in config', function(
	assert
) {
	const getVersion = this.stub(this.ads.utils, 'getVersion');
	getVersion.returns('x.y.z');
...
QUnit.test('social referrer', function(assert) {
	...
	assert.equal(
		result.socref,
		undefined,
		'No document referrer, socref key returns undefined'
	);
	...
	assert.equal(
		result.socref,
		'twi',
		'Already logged in, twitter should be mapped from t.co to twi'
	);
	...
	assert.equal(
		result.socref,
		'fac',
		'Already logged in, facebook should be mapped from facebook.com to fac'
	);

	referrer.returns(
		'http://www.linkedin.com/company/4697?trk=NUS-body-company-name'
	);
	this.ads.init({ socialReferrer: true });
	result = this.ads.targeting.get();
	assert.equal(
		result.socref,
		'lin',
		'Already logged in, linkedin should be mapped from linkedin.com to lin'
	);
	...
	assert.equal(
		result.socref,
		'dru',
		'Already logged in, drudge should be mapped from drudgereport.com to dru'
	);
	...
	assert.equal(
		result.socref,
		'dia',
		'Already logged in, drudge should be mapped from dianomi.com to dia'
	);
	...
	assert.equal(
		result.socref,
		'goo',
		'Already logged in, drudge should be mapped from google.com to goo'
	);
});
...
QUnit.test('timestamp', function(assert) {
	...
	assert.equal(
		result.ts,
		'20131118233007',
		'timestamp value returns correctly'
	);
	...
	assert.equal(
		result.ts,
		'20131119003007',
		'fast forward one hour and timestamp value returns correctly'
	);
});
