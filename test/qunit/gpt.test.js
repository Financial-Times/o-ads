...
QUnit.test('init', function(assert) {
	...
	assert.ok(
		window.googletag.cmd.length,
		'library setup is added to the command queue'
	);
...
QUnit.test('broadcast an event when GPT fails to load', function(assert) {
	...
	assert.ok(
		this.ads.utils.broadcast.calledWith('adServerLoadError', 'some error')
	);
});
...
QUnit.test('set page targeting', function(assert) {
	...
	assert.ok(
		googletag.pubads().setTargeting.calledWith('some', 'test'),
		'the params are queued with GPT'
	);
	assert.ok(
		googletag.pubads().setTargeting.calledWith('targeting', 'params'),
		'the params are queued with GPT'
	);
});
...
QUnit.test('override page targeting', function(assert) {
	...
	assert.ok(
		googletag
			.pubads()
			.setTargeting.neverCalledWith(
				'anotherOverrideKey',
				'anotherOverrideValue'
			),
...
QUnit.test(
	'override page targetting catches and warns when googletag is not available',
	function(assert) {
		...
		assert.ok(
			errorSpy.calledWith(
				'Attempting to set page targeting before the GPT library has initialized'
			),
...
QUnit.test('unset page targeting', function(assert) {
	...
	assert.ok(
		googletag.pubads().clearTargeting.calledWith('test'),
		'removals of custom params are queued with GPT'
	);
});
...
QUnit.test(
	...
	function(assert) {
		...
		assert.ok(
			errorSpy.calledWith(
				'Refusing to unset all keys - a key must be specified'
			),
		...
		assert.ok(
			errorSpy.calledWith(
				'Attempting to clear page targeting before the GPT library has initialized'
			),
			'warns that googletag is not available'
		);
...
QUnit.test('set sync rendering', function(assert) {
	...
	assert.ok(
		googletag.pubads().enableSyncRendering.calledOnce,
		'sync rendering has been enabled'
	);
});
...
QUnit.test('enabled single request', function(assert) {
	...
	assert.ok(
		googletag.pubads().enableSingleRequest.calledOnce,
		'single request has been enabled'
	);
});
...
QUnit.test('default to async rendering', function(assert) {
	...
	assert.ok(
		googletag.pubads().enableAsyncRendering.calledOnce,
		'async rendering has been enabled by default'
	);
});
...
QUnit.test('enables companion ads', function(assert) {
	...
	assert.ok(
		googletag.pubads().disableInitialLoad.calledOnce,
		'disabled the initial load'
	);
	assert.ok(googletag.companionAds.calledOnce, 'companion ads called');
	assert.ok(
		googletag.companionAds().setRefreshUnfilledSlots.calledOnce,
		'companion ads api called'
	);
	assert.ok(
...
QUnit.test(
	'set correct collapse mode when config collapseEmpty is after',
	function(assert) {
		this.fixturesContainer.add(
			'<div data-o-ads-name="mpu" data-o-ads-formats="MediumRectangle"></div>'
		);
		this.ads.init({ collapseEmpty: 'after' });
		const slot = this.ads.slots.initSlot('mpu');
		assert.ok(
			slot.gpt.slot.setCollapseEmptyDiv.calledOnce,
			'call collapse empty slot gpt api'
		);
		assert.ok(
...
QUnit.test(
	'set correct collapse mode when config collapseEmpty is before',
	function(assert) {
		this.fixturesContainer.add(
			'<div data-o-ads-name="mpu" data-o-ads-formats="MediumRectangle"></div>'
		);
...
QUnit.test(
	'set correct collapse mode when config collapseEmpty is never',
	function(assert) {
		this.fixturesContainer.add(
			'<div data-o-ads-name="mpu" data-o-ads-formats="MediumRectangle"></div>'
		);
...
QUnit.test('set collapse mode as "never" if no mode is specified', function(
	assert
) {
	this.fixturesContainer.add(
		'<div data-o-ads-name="mpu" data-o-ads-formats="MediumRectangle"></div>'
...
QUnit.test('component specific mode overrides config set parameter', function(
	assert
) {
	this.fixturesContainer.add(
...
QUnit.test(
	...
	function(assert) {
		...
		const slot = this.ads.slots.initSlot('mpu');
		assert.ok(
			slot.collapseEmpty === undefined,
			'collapse empty is not set when attribute value is wrong'
		);
		assert.ok(
			slot.gpt.slot.setCollapseEmptyDiv.calledOnce,
			'call collapse empty slot gpt api'
...
QUnit.test(
	'catches slot in view render event and display it if method is ready',
	function(assert) {
...
QUnit.test('on slot refresh event updates targeting', function(assert) {
	const slotHTML =
		'<div data-o-ads-formats="MediumRectangle" data-o-ads-targeting="random=1"></div>';
	const node = this.fixturesContainer.add(slotHTML);
	this.ads.init();
	const slot = this.ads.slots.initSlot(node);
	slot.fire('refresh', { targeting: { test: true } });
	assert.ok(
		googletag.pubads().refresh.calledOnce,
		'googletag refresh method has been called'
	);
	assert.ok(
		slot.gpt.slot.setTargeting.calledTwice,
		'setTargeting has been called on the slot'
	);
	assert.ok(
		slot.gpt.slot.setTargeting.calledWith('random', '1'),
		'setTargeting has been passed correct argument'
	);
	assert.ok(
		slot.gpt.slot.setTargeting.calledWith('test', true),
		'setTargeting has been passed correct argument'
	);
});
...
QUnit.test('provides api to destroy the slot', function(assert) {
	const slotHTML =
		'<div data-o-ads-name="test1" data-o-ads-formats="MediumRectangle"></div>';
	...
	assert.equal(
		slot.destroySlot(),
		true,
		'a call to destroy slot returns a boolean'
	);
	assert.ok(googletag.destroySlots.calledOnce, 'destroy api has been called');
	assert.ok(
		googletag.destroySlots.calledWith([slot.gpt.slot]),
		'defaults to slot that the method has been invoked on'
	);
});

QUnit.test(
	'destroying slots fails gracefully if pubadsReady is not available',
	function(assert) {
		const slotHTML =
			'<div data-o-ads-name="test1" data-o-ads-formats="MediumRectangle"></div>';
		this.fixturesContainer.add(slotHTML);
		...
		assert.equal(
			slot.destroySlot(),
			false,
			'a call to destroy slot returns false'
		);
		assert.ok(
			!googletag.destroySlots.called,
			'destroy api has not been called'
		);
...
QUnit.test('provides api to clear the slot', function(assert) {
	...
	assert.equal(
		slot.clearSlot(),
		true,
		'a call to clear slot returns a boolean'
	);
...
QUnit.test('provides api to clear another slot', function(assert) {
	...
	assert.ok(googletag.pubads().clear.calledOnce, 'clear api has been called');
	assert.ok(
		googletag.pubads().clear.calledWith([slot2.gpt.slot]),
		'one slot can clear '
	);
});
...
QUnit.test('companion service is enabled globally on slots', function(assert) {
	this.fixturesContainer.add(
		'<div class="o-ads" data-o-ads-name="test" data-o-ads-formats="MediumRectangle"></div>'
	);
	this.ads.init({ gpt: { companions: true } });
...
QUnit.test('companion service can be switched off per format', function(
	assert
) {
	this.fixturesContainer.add(
		'<div class="o-ads" data-o-ads-name="TestFormat" data-o-ads-formats="TestFormat"></div>'
	);
	this.ads.init({
		slots: {
			TestFormat: {
	...
	document.addEventListener('oAds.slotExpand', function(event) {
		...
		if (name === 'unit-name-full') {
			assert.strictEqual(
				slot.gpt.unitName,
				expected,
				'setting unit name with site and zone works'
			);
			done();
		}
	});
	...
	document.addEventListener('oAds.slotExpand', function(event) {
		...
		if (name === 'unit-name-site-only') {
			assert.strictEqual(
				slot.gpt.unitName,
				expected,
				'setting unit name with site and no zone works'
			);
			done();
		}
	});
...
QUnit.test('set unit name with attribute', function(assert) {
	...
	const container = this.fixturesContainer.add(
		htmlstart +
			'unit-name-attr" data-o-ads-gpt-unit-name="' +
			expected +
			htmlend
	);
...
QUnit.test('collapse empty', function(assert) {
	this.ads.init({ collapseEmpty: 'after' });
	assert.ok(
		googletag.pubads().collapseEmptyDivs.calledWith(false),
		'after mode is set in gpt'
	);
	googletag.pubads().collapseEmptyDivs.reset();

	this.ads.init({ collapseEmpty: 'before' });
	assert.ok(
		googletag.pubads().collapseEmptyDivs.calledWith(true),
		'before mode is set in gpt'
	);
	googletag.pubads().collapseEmptyDivs.reset();

	this.ads.init({ collapseEmpty: 'never' });
	assert.ok(
		googletag.pubads().collapseEmptyDivs.notCalled,
		'never mode is set in gpt'
	);
});
...
QUnit.skip('submit impression', function(assert) {
	...
	assert.ok(
		infoLog.calledWith('Impression Url requested'),
		'correct impression info logged'
	);
...
QUnit.skip('catches a failure to submit an impression', function(assert) {
	...
	const attachSpy = this.stub(this.ads.utils, 'attach', function(
		url,
		async,
		successCallback,
		failureCallback
	...
	slot.submitGptImpression();
	assert.ok(
		attachSpy.calledWith('https://www.ft.com'),
		'impression url requested via utils'
	);
	assert.ok(
...
QUnit.test(
	'logs a warning when trying to submit an impression and the URL is not present within creative',
	function(assert) {
		...
		this.fixturesContainer.add(html);
		this.ads.init();
		const slot = this.ads.slots.initSlot(
			'delayedimpression-missing-tracking-div'
		);
		slot.submitGptImpression();
		assert.notOk(
			this.ads.utils.attach.calledWith('https://www.ft.com'),
			'impression url never requested via utils'
		);
		assert.ok(
			log.calledWith(
				'Impression URL not found, this is set via a creative template.'
			),
...
QUnit.test(
	'logs a warning when trying to submit an impression on a non-oop slot',
	function(assert) {
		const log = this.spy(this.utils.log, 'warn');
		const html =
			'<div data-o-ads-name="delayedimpression" data-o-ads-formats="MediumRectangle"></div>';
		this.fixturesContainer.add(html);
		this.ads.init();
		const slot = this.ads.slots.initSlot('delayedimpression');
		slot.submitGptImpression();
...
QUnit.test('define a basic slot', function(assert) {
	const html =
		'<div data-o-ads-name="no-responsive-mpu" data-o-ads-formats="MediumRectangle"></div>';
	this.fixturesContainer.add(html);
	...
	assert.equal(
		gptSlot.defineSizeMapping.callCount,
		0,
		'the GPT defineSizeMapping slot is not called'
	);
});
...
QUnit.test('define responsive slot', function(assert) {
	...
	assert.ok(
		gptSlot.defineSizeMapping.calledOnce,
		'the GPT defineSizeMapping slot is called'
	);
});

QUnit.test(
	'slotRenderStart event is fired with the correct slot name',
	function(assert) {
		const done = assert.async();
		const html =
		...
		document.body.addEventListener('oAds.slotRenderStart', function(event) {
			assert.equal(
				event.detail.name,
				'rendered-test',
				'our test slot fired the slotRenderStart event'
			);
			done();
		});
...
QUnit.test('fixed url for ad requests', function(assert) {
	this.fixturesContainer.add(
		'<div data-o-ads-name="url-slot" data-o-ads-formats="MediumRectangle"></div>'
	);
	this.ads.init({
	...
	assert.ok(
		gptSlot.set.calledWith('page_url', 'http://www.ft.com'),
		'page url set via config'
	);

	document.head.insertAdjacentHTML(
		'beforeend',
		'<link href="http://www.random-inc.com/" rel="canonical" remove />'
	);
	this.fixturesContainer.add(
		'<div data-o-ads-name="canonical-slot" data-o-ads-formats="MediumRectangle"></div>'
	);
	this.ads.init();
	...
	assert.ok(
		gptSlot.set.calledWith('page_url', 'http://www.random-inc.com/'),
		'page url set via canonical link tag'
	);
});

QUnit.test(
	'pick up the slot URL from page address if config or canonical not available',
	function(assert) {
...
QUnit.test('creatives with size 100x100 expand the iframe to 100%', function(
	assert
) {
	const done = assert.async();
	document.body.addEventListener('oAds.slotExpand', function(event) {
		...
		assert.deepEqual(
			iframeSize,
			['100%', '100%'],
			'size of iframe is 100% by 100%.'
		);
		done();
	});

	const slotHTML =
		'<div data-o-ads-name="fullpage" data-o-ads-sizes="100x100"></div>';
	const node = this.fixturesContainer.add(slotHTML);
	this.ads.init({});
...
QUnit.test('command queue empty when googletag is available', function(assert) {
	...
	assert.equal(
		window.googletag.cmd.length,
		0,
		'command queue empty when googletag is available'
	);
});

QUnit.test(
	'setup added to command queue when googletag not available',
	function(assert) {
		// delete the mock for this test
		delete window.googletag;
...
QUnit.test(
	'defineSlot is added to command queue when googletag is not available',
	function(assert) {
		// delete the mock for this test
		delete window.googletag;
		...
		assert.equal(
			window.googletag.cmd.length,
			gptCommandsQueued + 1,
			'defineSlot function added to command queue'
		);
...
QUnit.test('clearSlot returns false when googletag is not available', function(
	assert
) {
	// delete the mock for this test
	delete window.googletag;
	...
	assert.equal(
		slot.clearSlot(),
		false,
		'clearSlot function added to command queue'
	);
	// reinstate mock
	window.googletag = this.gpt;
});
...
QUnit.test(
	'initResponsive is added to command queue when googletag is not available',
	function(assert) {
		// delete the mock for this test
		delete window.googletag;
		...
		assert.equal(
			window.googletag.cmd.length,
			gptCommandsQueued + 1,
			'initResponsive function added to command queue'
		);
...
QUnit.test(
	'display is added to command queue when googletag is not available',
	function(assert) {
		// delete the mock for this test
		delete window.googletag;
		...
		assert.equal(
			window.googletag.cmd.length,
			gptCommandsQueued + 1,
			'display function added to command queue'
		);
...
QUnit.test(
	'setUnitName is added to command queue when googletag is not available',
	function(assert) {
		// delete the mock for this test
		delete window.googletag;
		...
		assert.equal(
			window.googletag.cmd.length,
			gptCommandsQueued + 1,
			'setUnitName function added to command queue'
		);
...
QUnit.test(
	'addServices is added to command queue when googletag is not available',
	function(assert) {
		// delete the mock for this test
		delete window.googletag;
		...
		assert.equal(
			window.googletag.cmd.length,
			gptCommandsQueued + 1,
			'addServices function added to command queue'
		);
...
QUnit.test(
	'setCollapseEmpty is added to command queue when googletag is not available',
	function(assert) {
		// delete the mock for this test
		delete window.googletag;
		...
		assert.equal(
			window.googletag.cmd.length,
			gptCommandsQueued + 1,
			'setCollapseEmpty function added to command queue'
		);
		// reinstate mock
		window.googletag = this.gpt;
	}
...
QUnit.test(
	'setURL is added to command queue when googletag is not available',
	function(assert) {
		// delete the mock for this test
		delete window.googletag;
		...
		assert.equal(
			window.googletag.cmd.length,
			gptCommandsQueued + 1,
			'setURL function added to command queue'
		);
		// reinstate mock
		window.googletag = this.gpt;
	}
...
QUnit.test(
	'setTargeting is added to command queue when googletag is not available',
	function(assert) {
		// delete the mock for this test
		delete window.googletag;
		...
		assert.equal(
			window.googletag.cmd.length,
			gptCommandsQueued + 1,
			'setTargeting function added to command queue'
		);
		// reinstate mock
		window.googletag = this.gpt;
	}
...
QUnit.test(
	'updateCorrelator is added to command queue when googletag is not available',
	function(assert) {
		// delete the mock for this test
		delete window.googletag;
		...
		assert.equal(
			window.googletag.cmd.length,
			gptCommandsQueued + 1,
			'updateCorrelator function added to command queue'
		);
		// reinstate mock
		window.googletag = this.gpt;
	}
...
QUnit.test(
	'clear slot method calls to the ad server providers clear function',
	function(assert) {
		const slotHTML = '<div data-o-ads-formats="MediumRectangle"></div>';
		const node = this.fixturesContainer.add(slotHTML);
		this.ads.init();
...
QUnit.test(
	'onRefresh is added to command queue when googletag is not available',
	function(assert) {
		// delete the mock for this test
		delete window.googletag;
		...
		assert.equal(
			window.googletag.cmd.length,
			gptCommandsQueued + 1,
			'onRefresh function added to command queue'
		);
...
QUnit.test('nonPersonalized request mode', function(assert) {
	document.cookie =
		'FTConsent=behaviouraladsOnsite%3Aoff%2CprogrammaticadsOnsite%3Aon;';
	this.ads.init({ gpt: true });
	assert.ok(
