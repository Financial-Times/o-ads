...
QUnit.test('set page targeting', function (assert) {
	...
	var result = this.ads.gpt.setPageTargeting();
	var expected = {some: 'test', targeting: 'params'};
	assert.deepEqual(result, expected, 'setting dfp_targeting in config works');
...
QUnit.test('define responsive slot', function (assert) {
	this.fixturesContainer.insertAdjacentHTML('beforeend', '<div id="responsive-mpu"></div>');
	this.ads.init({
		responsive:{
			large: [1280, 800],
			medium: [970, 690],
	...
	var gptSlot = this.ads.slots['responsive-mpu'].gptSlot;
...
QUnit.test('define non reponsive slot', function (assert) {
	this.fixturesContainer.insertAdjacentHTML('beforeend', '<div id="no-responsive-mpu"></div>');
	this.ads.init({
		responsive: {
			large: [1280, 800],
			medium: [970, 690],
	...
	var gptSlot = this.ads.slots['no-responsive-mpu'].gptSlot;
...
QUnit.test('fix the url for ad requests', function (assert) {
	this.fixturesContainer.insertAdjacentHTML('beforeend', '<div id="url-slot"></div>');
	this.ads.init({
	...
	var slot = this.ads.slots['url-slot'];
	assert.ok(slot.gptSlot.set.calledWith('page_url', 'http://www.ft.com'), 'page url set via config');

	document.head.insertAdjacentHTML('beforeend', '<link href="http://www.ft.com/" rel="canonical" remove />');
	this.fixturesContainer.insertAdjacentHTML('beforeend', '<div id="canonical-slot"></div>');
	this.ads.init({
		formats: {
			'canonical-slot': {
				sizes: [[300, 250]]
	...
	slot = this.ads.slots['canonical-slot'];
	assert.ok(slot.gptSlot.set.calledWith('page_url', 'http://www.ft.com/'), 'page url set via canonical link tag');
});
