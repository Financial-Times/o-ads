...
QUnit.test('Config get/set', function(assert) {
	...
	assert.deepEqual(
		result,
		undefined,
		'passing an invalid key returns undefined.'
	);
	...
	assert.deepEqual(
		result,
		value2,
		'set an existing key returns the new value.'
	);
...
QUnit.test('Config defaults', function(assert) {
	...
	assert.ok(
		result.hasOwnProperty('flags'),
		'default properties have been added to config'
	);
	assert.deepEqual(
		this.ads.config('flags'),
		flags,
		'Config returns the correct value'
	);
});

QUnit.test('Config fetchDeclaritive', function(assert) {
	const save = window.JSON;
	this.fixturesContainer
		.get()
		.insertAdjacentHTML(
			'beforeend',
			'<script data-o-ads-config type="application/json">{"dfpsite" : "site.site","dfpzone" : "zone.zone"}</script>'
		);
	this.ads.init();
	let result = this.ads.config();
	assert.ok(
		result.dfpzone,
		'Config has been fetched from the inline declarative script'
	);
	...
	assert.notOk(
		result.dfpsite,
		'no DFP Site - when JSON not available the declarative config is not parsed'
	);
	assert.notOk(
		result.dfpzone,
		'no DFP zone - when JSON not available the declarative config is not parsed'
	);
	window['JSON'] = save;
});
...
QUnit.test('Config fetchDeclaritive, multiple script tags', function(assert) {
	this.fixturesContainer
		...
		.insertAdjacentHTML(
			'beforeend',
			'<script data-o-ads-config type="application/json">{"athing" : "thing", "anotherthing" : "another"}</script>'
		);
	this.fixturesContainer
		.get()
		.insertAdjacentHTML(
			'beforeend',
			'<script data-o-ads-config type="application/json">{"more" : "evenmore"}</script>'
		);
	this.ads.init();
	const result = this.ads.config();
...
QUnit.test(
	"Config deep extends so default options like formats aren't overwritten",
	function(assert) {
		this.ads.init();
		const result = this.ads.config({
			formats: { someNewFormat: { sizes: [[2, 2]] } }
		});
...
QUnit.test(
	'Config works as expected even when there are custom prototype methods defined (e.g. polyfill)',
	function(assert) {
		...
		assert.deepEqual(
			this.ads.config('responsive'),
			repsonsiveDefaults,
			'Config returns the correct values for responsive slots'
		);
