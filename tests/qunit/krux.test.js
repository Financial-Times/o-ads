...
describe('Krux', function () {
	...
	it("control tag is attached when initialised", function () {
	...
	it('sets global Krux if one is not available yet', function () {
		delete window.Krux;
		this.ads.init({ krux: { id: '112233', attributes: { page: { uuid: '123' } } } });
		assert.ok(window.Krux, 'Krux is set in the init');
	});

	it('adds a script from url when location contains krux src', function () {
	...
	it('adds Krux script after a timeout if requestIdleCallback doesn\'t exist', function () {
		this.stub(this.utils, 'getLocation').returns('http://domain/?kxsrc=http%3A%2F%2Fcdn.krxd.net%3A123%2F');
		this.stub(this.utils, 'broadcast');
	...
	it('does not add a script from url when location contains krux src that is set to disable', function () {
	...
	it('targeting data is generated correctly from localStorage', function () {
	...
	it('targeting data is generated correctly from cookies', function () {
	...
	it('number of Krux segments can be limited', function () {
		document.cookie = "kxsegs=seg1,seg2,seg3,seg4; kxuser=kxuser";
	...
	it('event pixels event not fire if no id is passed', function () {
		this.ads.krux.events.fire();
		assert.notOk(window.Krux.calledOnce, 'does not fire and event');
	});

	it('events on DOM elements ', function () {
	...
	it('page attributes are set correctly and sent to Krux', function () {
		this.ads.init({ krux: { id: '112233', attributes: { page: { uuid: '123' } } } });

		assert.ok(window.Krux.withArgs("set", "page_attr_uuid", "123").calledOnce, "page attributes sent to Krux");
	});

	it('user attributes are set correctly and sent to Krux', function () {
	...
	it('other attributes are set correctly and sent to Krux', function () {
	...
	it('page & user attributes are set correctly and sent to Krux', function () {
	...
	it('debug starts logging Krux data', function () {
		this.ads.init({ krux: {} });
		const start = this.spy(this.utils.log, 'start');
	...
	it("debug doesn't log attributes data if none is set", function () {
		this.ads.init({ krux: {} });
		const start = this.spy(this.utils.log, 'start');
	...
	it("debug logs attributes data if set", function () {
	...
	it("debug doesn't log events data if none is set", function () {
		this.ads.init({ krux: {} });
		const start = this.spy(this.utils.log, 'start');
	...
	it("debug logs events data if set", function () {
	...
	it('debug logs number of supertag scripts', function () {
	...
	it("debug logs segment limit data if set", function () {
	...
	it("adds data to config", function () {
		const input = { "key": "value" };
		this.ads.krux.add(input);
		assert.deepEqual(this.ads.krux.customAttributes, input, 'win');
	});

	it('customAttributes are sent even if no config attributes are', function () {
	...
	it('sendNewPixel sends a new pixel for page loads', function () {
		this.ads.init({ krux: { id: '112233' } });
	...
	it('sendNewPixel sends a new pixel for non-page loads', function () {
		this.ads.init({ krux: { id: '112233' } });
		const pixelStub = this.stub();
		window.Krux.returns({ send: pixelStub });
	...
	it('resetAttributes resets all attributes', function () {
	...
	it('resetSpecificAttributes resets just that attribute', function () {
		this.ads.krux.add({ user: { "key": "value" }, page: { "key1": "value", "key2": false }, custom: { "key": "value" } });
