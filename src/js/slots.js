...
function onBreakpointChange(slots, screensize) {
	slots.forEach(function (slot) {
		...
		if (slot) {
			...
			const stillWantsResponsive =
				slot.sizes[screensize] &&
				slot.sizes[screensize].filter((size) => findFormatBySize(size) === 'Responsive').length > 0;
...
Slots.prototype.initLazyLoading = function (slotConfig) {
	...
	function onChange(changes) {
		...
		changes
			.filter(
				(a) =>
					a.intersectionRect.height || a.intersectionRect.width || a.intersectionRect.top || a.intersectionRect.left
			)
			...
			sizes:
				utils.isArray(slot.sizes) &&
					slot.sizes
						...
						.join(', ') ||
				'responsive slot',
			targeting: Object.keys(slot.targeting)
				.map(function (param) {
