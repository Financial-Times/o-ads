...
function setup(gptConfig) {
	...
	const url = stripUrlParams({ href: window.location, filters: SEARCH_PARAMS });
...
function onRenderEnded(event) {
	...
	const slotTargeting =
		event.slot.getTargetingMap && event.slot.getTargetingMap();
...
const slotMethods = {
		...
		window.googletag.cmd.push(() => {
			...
			if (!this.outOfPage) {
				if (breakpoints && utils.isObject(this.sizes)) {
					this.initResponsive();
					this.gpt.slot = googletag
						.defineSlot(this.gpt.unitName, [], this.gpt.id)
						.defineSizeMapping(this.gpt.sizes);
				} else {
					this.gpt.slot = googletag.defineSlot(
						this.gpt.unitName,
						this.sizes,
						this.gpt.id
					);
				...
				this.gpt.slot = googletag.defineOutOfPageSlot(
					this.gpt.unitName,
					this.gpt.id
				);
			}
	...
	setCollapseEmpty: function() {
		window.googletag.cmd.push(() => {
			const mode =
				this.collapseEmpty || config('collapseEmpty') || DEFAULT_COLLAPSE_MODE;
	...
	submitGptImpression: function() {
		...
		function getImpressionURL(iframe) {
			const trackingUrlElement = iframe.contentWindow.document.querySelector(
				'[data-o-ads-impression-url]'
			);
